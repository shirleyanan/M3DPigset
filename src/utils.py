...
def load_scene(fpath, verbose=True):
    with open(fpath, 'r') as f:
        data = json.load(
            f)  # D:\python_code\3Dproject\AcinoSet\PigSet\Experiments\2019_03_09\extrinsic_calib\6_cam_scene_sba .json
        cam_res = tuple(data['camera_resolution'])
        k_arr = []
        d_arr = []
        r_arr = []
...
def load_dlc_points_as_df(dlc_df_fpaths, verbose=True):
    ...
    for path in dlc_df_fpaths:
        ...
        dlc_df = dlc_df.droplevel([0], axis=1).swaplevel(0, 1, axis=1).T.unstack().T.reset_index().rename(
            {'level_0': 'frame'}, axis=1)
        dlc_df.columns.name = ''
        dfs.append(dlc_df)
    # create new dataframe
    dlc_df = pd.DataFrame(columns=['frame', 'camera', 'marker', 'x', 'y', 'likelihood', 'frame_index'])
    for i, df in enumerate(dfs):
        df['camera'] = int(dlc_df_fpaths[i].split('\\')[-1][3])
        df.rename(columns={'bodyparts': 'marker'}, inplace=True)
        group_size = 26
        df['frame_index'] = (df.index // group_size).astype(int)
        dlc_df = pd.concat([dlc_df, df], sort=True, ignore_index=True)

    dlc_df = dlc_df[['frame', 'camera', 'marker', 'x', 'y', 'likelihood', 'frame_index']]
    if verbose:
...
def save_3d_cheetah_as_2d(positions_3d, out_dir, scene_fpath, bodyparts, project_func, start_frame, save_as_csv=True,
                          out_fname=None):
    assert os.path.dirname(
        os.path.dirname(scene_fpath)) in out_dir, 'scene_fpath does not belong to the same parent folder as out_dir'

    video_fpaths = sorted(
        glob(os.path.join(os.path.dirname(out_dir), 'videos', 'cam[1-9].mp4')))  # check current dir for videos
    if not video_fpaths:
        video_fpaths = sorted(
            glob(os.path.join(os.path.dirname(out_dir), 'cam[1-9].mp4')))  # check parent dir for videos
    ...
    if video_fpaths:
        ...
        for i in range(len(video_fpaths)):
            ...
            df = pd.DataFrame(data.reshape((n_frames, -1)), columns=pdindex,
...
def find_scene_file(dir_path, scene_fname=None, verbose=True):
    if scene_fname is None:
        ...
        scene_fname = f'{n_cams}_cam_scene.json' if n_cams else '[1-9]_cam_scene*.json'
        print(scene_fname)
