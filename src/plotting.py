...
from .utils import load_scene, create_board_object_pts

pg.setConfigOptions(antialias=True)

def plot_linregress( frames, x, y, z, x_slope, x_intercept, y_slope, y_intercept, z_slope, z_intercept, position_name, plot=False):
    if plot:
        plt.title(position_name+'_x')
        plt.scatter(frames, x, s=5, c='b')
        plt.plot(frames, x_slope * frames + x_intercept,'r')
        plt.xlabel('frames')
        plt.ylabel('x')
        plt.show()

        plt.title(position_name+'_y')
        plt.scatter(frames, y, s=5, c='b')
        plt.plot(frames, y_slope * frames + y_intercept, 'r')
        plt.xlabel('frames')
        plt.ylabel('y')
        plt.show()

    plt.title(position_name+'_z')
    plt.scatter(frames, z, s=5, c='b')
    plt.plot(frames, z_slope * frames + z_intercept, 'r')
    plt.xlabel('frames')
    plt.ylabel('z')
    plt.show()
...
class Animation:
    def __init__(self, title, scene_fpath, centered=False, dark_mode=True):
        # 创建一个应用程序对象
...
#         self.view.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        self.view.opts['distance'] = 15  # 初始视角高度15
        self.view.orbit(-135, 10)  # 初始视角-135,10   -20
...
class Cheetah(Animation):
    def __init__(self, multiple_reconstructions, scene_fpath,labels, project_func, hide_lure=False, reprojections=True, **kwargs):
        ...
        # 连线 indices correspond to joints in 'markers' variable
        # pig skeleton
        lines_idxs = [3, 0, 3, 1, 3, 2, 3, 4,
                      4, 5, 5, 6,
                      5, 7, 7, 8, 8, 9, 9, 10,
        ...
        if self.reprojections:
            self.cam_data, self.cam_lines, self.cam_img, cam_w = [], [], [], []
